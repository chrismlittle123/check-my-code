...
`;
  ...
  afterEach(async () => {
    ...
    await writeFile(join(forceOverwriteDir, 'eslint.config.js'), eslintOriginal);
  ...
  it('overwrites ESLint config when --force is used', async () => {
    ...
    const result = await run('generate/force-overwrite', ['generate', 'eslint', '--force']);
    expect(result.exitCode).toBe(0);
  ...
  it('overwrites Ruff config when --force is used', async () => {
    ...
    // Run generate with --force
    const result = await run('generate/force-overwrite', ['generate', 'ruff', '--force']);
    expect(result.exitCode).toBe(0);
...
describe('cmc generate tsc', () => {
  it('generates valid tsconfig.json with --stdout', async () => {
    const result = await run('generate/tsc/fresh', ['generate', 'tsc', '--stdout']);
