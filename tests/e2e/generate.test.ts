...
// =============================================================================
describe("cmc generate claude", () => {
  const freshDir = join(PROJECTS_DIR, "generate", "claude", "fresh");
  const existingDir = join(PROJECTS_DIR, "generate", "claude", "existing");
  const existingOriginal = `{
  "$schema": "https://raw.githubusercontent.com/anthropics/claude-code/main/.claude/settings-schema.json",
  "_comment": "OLD SETTINGS - should be overwritten by --force",
  "permissions": {
    "allow": ["Read"],
    "deny": []
  }
...
`;
  ...
  afterEach(async () => {
    // Clean up generated settings in fresh project
    try {
      await rm(join(freshDir, ".claude"), { recursive: true, force: true });
    } catch {
      // Ignore if doesn't exist
    }
    // Restore original settings in existing project
    await writeFile(
      join(existingDir, ".claude", "settings.json"),
      existingOriginal,
    );
  });

  it("generates valid Claude settings with --stdout", async () => {
    const result = await run("generate/claude/fresh", [
      "generate",
      "claude",
      "--stdout",
    ]);
    ...
    expect(result.exitCode).toBe(0);
    const config = JSON.parse(result.stdout);
    expect(config.$schema).toContain("settings-schema.json");
    expect(config._comment).toContain("Generated by cmc");
    expect(config.permissions).toBeDefined();
    expect(config.permissions.allow).toBeInstanceOf(Array);
  });

  it("includes permissions from remote config", async () => {
    const result = await run("generate/claude/fresh", [
      "generate",
      "claude",
      "--stdout",
  ...
  it("fails when settings.json exists without --force", async () => {
    const result = await run("generate/claude/existing", [
      "generate",
      "claude",
    ]);
  ...
  it("overwrites settings.json when --force is used", async () => {
    const settingsPath = join(existingDir, ".claude", "settings.json");
    const originalContent = await readFile(settingsPath, "utf-8");
    expect(originalContent).toContain("OLD SETTINGS");
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("Generated .claude/settings.json");

    const newContent = await readFile(settingsPath, "utf-8");
    expect(newContent).toContain("Generated by cmc");
    expect(newContent).not.toContain("OLD SETTINGS");
  });

  it("fails with exit code 2 when no [ai.claude] configured", async () => {
    const result = await run("generate/claude/no-config", [
      "generate",
      "claude",
    ]);
  ...
  it("is case insensitive for target name", async () => {
    const result = await run("generate/claude/fresh", [
      "generate",
      "CLAUDE",
      "--stdout",
