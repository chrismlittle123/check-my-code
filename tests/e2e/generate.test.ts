...
// =============================================================================
// Generate: Empty rulesets
// =============================================================================
describe.skipIf(!dockerAvailable)('cmc generate - empty rulesets', () => {
  it('handles ESLint generation with no rules defined', async () => {
    const result = await runInDocker(images['generate/empty-ruleset'], [
      'generate',
      'eslint',
      '--stdout',
    ]);

    // Should still generate a valid config, just with no custom rules
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('// Generated by cmc');
  ...
  it('handles Ruff generation with no config defined', async () => {
    ...
    // Should still generate a valid config with defaults
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('# Generated by cmc');
  }, 30000);
});
