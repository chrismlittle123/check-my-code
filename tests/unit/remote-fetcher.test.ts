...
describe('parseRemoteRef', () => {
  ...
  describe('invalid references', () => {
    it('throws on missing host prefix', () => {
      expect(() => parseRemoteRef('owner/repo@v1.0.0')).toThrow(RemoteFetchError);
    });

    it('throws on unsupported host', () => {
      expect(() => parseRemoteRef('gitlab:owner/repo@v1.0.0')).toThrow(RemoteFetchError);
    });

    it('throws on missing version', () => {
      expect(() => parseRemoteRef('github:owner/repo/path')).toThrow(RemoteFetchError);
    });

    it('throws on missing repo', () => {
      expect(() => parseRemoteRef('github:owner@v1.0.0')).toThrow(RemoteFetchError);
    });
    ...
    it('error message includes expected format', () => {
      try {
        ...
        expect((e as Error).message).toContain('github:owner/repo/path@version');
      }
    });
  });
});
