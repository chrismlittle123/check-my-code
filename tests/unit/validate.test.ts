...
`
      ...
      expect(result.errors.some((e) => e.message.includes('project'))).toBe(true);
    });
...
`
    ...
    it('rejects invalid TOML syntax', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.message.includes('TOML syntax'))).toBe(true);
    });
...
`
  ...
  describe('file not found', () => {
    it('throws error for non-existent file', async () => {
      ...
      await expect(validateConfig('/nonexistent/path/cmc.toml')).rejects.toThrow('not found');
    });
  });
});
